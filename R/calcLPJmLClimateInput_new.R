#' @title calcLPJmLClimateInput_new
...
calcLPJmLClimateInput_new <- function(climatetype = "MRI-ESM2-0:ssp370", # nolint
                                      variable = "temperature:annualMean",
  ...
  if (grepl("raw|smoothed", stage)) {
    ...
    if (!grepl("historical", climatetype)) {
      ...
      x <- mbind(readSource("LPJmLClimateInput_new", subtype = .subtypeHist,
                            subset = var$timeres, convert = "onlycorrect"),
                 readSource("LPJmLClimateInput_new", subtype = .subtypeScen,
                            subset = var$timeres, convert = "onlycorrect"))
      ...
      x     <- readSource("LPJmLClimateInput_new", subtype = .subtypeHist,
                          subset = var$timeres, convert = "onlycorrect")
      years <- getYears(x, as.integer = TRUE)
    ...
    if (stage == "harmonized") {
      ...
      baseline <- calcOutput("LPJmLClimateInput_new", climatetype = cfg$baselineHist,
                             variable = variable, stage = "smoothed",
                             lpjmlVersion = lpjmlVersion, aggregate = FALSE)
      x        <- calcOutput("LPJmLClimateInput_new", climatetype = cfg$climatetype,
                             variable = variable, stage = "smoothed",
                             lpjmlVersion = lpjmlVersion, aggregate = FALSE)
      ...
      # read in historical data for subtype
      baseline2020 <- calcOutput("LPJmLClimateInput_new", climatetype = cfg$baselineGcm,
